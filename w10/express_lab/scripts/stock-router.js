...
const stocks = provider.data; // return all the stocks when a root request arrives 
...
const handleSingleSymbol = (app) => {
    app.get('/stock/:symbol', (req, resp) => {
        ...
        const matches = stocks.filter(s => symbolToFind ===
            s.symbol);       // return the matching stock 
        if (matches.length > 0) {
            resp.json(matches);
        } else { resp.json(jsonMessage(`Symbol ${symbolToFind} not found`)); }
    });
...
const handleNameSearch = (app) => {
    app.get('/stock/name/:substring', (req, resp) => {
        ...
        const matches = stocks.filter(s =>
            s.name.toLowerCase().includes(substring));        // return the matching stocks 
        if (matches.length > 0) {
            resp.json(matches);
        } else { resp.json(jsonMessage(`No symbol matches found for ${substring}`)); }
    });
}
module.exports = {
    handleAllStocks,
