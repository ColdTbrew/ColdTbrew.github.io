...
const server = http.createServer(async (req, resp) => {
    ...
    let urlFile = url.parse(req.url).pathname;    // if no file provided in request, default to index.html   
    if (urlFile.length == 1) urlFile = "/index.html";
    console.log("Filename in URL=" + urlFile);    // turn it into the actual file system filename  
    ...
    try {
        const contents = await
            fsProm.readFile(localFile)
                .then(contents => {       // based on the URL path, extract the file extension 
                    ...
                    const header = { "Content-type": mime[1] || "text/plain" }; resp.writeHead(200, header);
                    // output the content of file 
                    resp.write(contents); resp.end();
                }).catch(err => { output500Error(resp); });
    }
    catch (err) {
        output500Error(resp);
    }
